...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Photo
      .findOne(_.pick(params, _.keys(Photo.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Photo.create(data);

    // Create relational data and return the entry.
    return Photo.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Photo.associations.map(a => a.alias));
    const data = _.omit(values, Photo.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Photo.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Photo.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Photo.attributes).reduce((acc, curr) => {
      switch (Photo.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
